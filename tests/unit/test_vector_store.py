...
class VectorStoreSettings(BaseModel):
    ...
    multi_path: str = Field(
        default_factory=lambda: os.environ.get("MULTI_VECTOR_STORE_PATH", "qdrant_audi_vector_store2")
    )
    multi_text_collection: str = Field(
        default_factory=lambda: os.environ.get("MULTI_TEXT_COLLECTION_NAME", "audi_spec_docs2")
    )
...
class TestVectorStoreExists:
    def test_storage_path_exists_on_disk(self, settings: VectorStoreSettings, store_path_exists: bool) -> None:
        assert store_path_exists, f"Vector store path '{settings.path}' does not exist - has ingestion been run yet?"
...
class TestCollectionsAreQueryable:
    ...
    def test_text_collection_has_points_and_returns_a_sample(
        ...
        assert "page_content" in payload, f"Expected 'page_content' in payload. Got keys: {list(payload.keys())}"
        assert "metadata" in payload, f"Expected 'metadata' in payload. Got keys: {list(payload.keys())}"
...
class TestMultiCollectionsAreQueryable:
    def test_multi_text_collection_has_points_and_returns_a_sample(
        ...
        points, _ = multi_client.scroll(collection_name=settings.multi_text_collection, limit=1, with_payload=True)
        assert len(points) == 1
    ...
    def test_multi_image_collection_has_points_and_returns_a_sample(
        ...
        if not info.points_count:
            pytest.skip(f"'{settings.multi_image_collection}' exists but has 0 points (no images were ingested).")

        points, _ = multi_client.scroll(collection_name=settings.multi_image_collection, limit=1, with_payload=True)
        assert len(points) == 1
